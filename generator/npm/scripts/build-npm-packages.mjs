...
//   - one @sightmap/sightkick-<os>-<arch> package per platform (binary + os/cpu)
//   - the @sightmap/sightkick meta package (bin launcher + optionalDependencies)
...
// stripped); --dist defaults to generator/dist; --out defaults to <repo>/npm-staging.
...
const metaDir = resolve(here, ".."); // generator/npm
const repoRoot = resolve(metaDir, "..", ".."); // repo root (generator/npm -> generator -> repo)
...
const distDir = resolve(arg("dist", join(repoRoot, "generator", "dist")));
...
const targets = [
  { key: "darwin-arm64", os: "darwin", cpu: "arm64", archive: "sightkick_darwin_arm64.tar.gz", bin: "sightkick" },
  { key: "darwin-x64", os: "darwin", cpu: "x64", archive: "sightkick_darwin_amd64.tar.gz", bin: "sightkick" },
  { key: "linux-arm64", os: "linux", cpu: "arm64", archive: "sightkick_linux_arm64.tar.gz", bin: "sightkick" },
  { key: "linux-x64", os: "linux", cpu: "x64", archive: "sightkick_linux_amd64.tar.gz", bin: "sightkick" },
  { key: "win32-arm64", os: "win32", cpu: "arm64", archive: "sightkick_windows_arm64.zip", bin: "sightkick.exe" },
  { key: "win32-x64", os: "win32", cpu: "x64", archive: "sightkick_windows_amd64.zip", bin: "sightkick.exe" },
];

// findArchive locates an archive by name under distDir, tolerating whatever
// nesting the CI artifact download produces (flat, or preserved under dist/).
...
for (const t of targets) {
  const name = `${SCOPE}/sightkick-${t.key}`;
  ...
  const pkg = {
    name,
    version,
    description: `sightkick native binary for ${t.key}`,
...
// pin version and the optionalDependencies to this release. The skills also ship
// in the meta package (see the "skills/" entry in package.json's files); the CLI
// itself installs them from the embedded copy via `sightkick skills install`.
